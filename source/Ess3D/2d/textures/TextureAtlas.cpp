...
namespace Ess3D {
  ...
  TextureAtlas::TextureAtlas(TextureCache* textureCache, std::string texturePath, std::string metadataPath) {
    Texture2D texture = textureCache->getTexture(texturePath);
  ...
  void TextureAtlas::loadMetadata(std::string metadataPath) {
    ...
    if (!fileStream.is_open()) {
      throw Ess3D::ERuntimeException("Failed to open " + metadataPath);
    }
    ...
    if (!document.IsArray()) {
      throw Ess3D::ERuntimeException("Texture atlas metadata file invalid.");
    }
