...
namespace Ess3D {
  ...
  void Shader::loadShader(ShaderType type, const std::string& shaderFilePath) {
    if (_shaderIds[type] != 0) {
      throw Ess3D::ERuntimeException("Already loaded another shader of the same type.");
    }

    _shaderIds[type] = glCreateShader(type);

    if (_shaderIds[type] == 0) {
      throw Ess3D::ERuntimeException("Failed to create shader id");
    }
    ...
    if (fileStream.is_open()) {
      ...
      throw Ess3D::ERuntimeException("Cannot load shader file: " + shaderFilePath);
    }
  ...
  void Shader::compileShaders() {
    for (auto it = _shaderIds.begin(); it != _shaderIds.end(); ++it) {
      ...
      if (compileSuccess == GL_FALSE) {
        ...
        std::printf("%s\n", &errorLog[0]);
        throw Ess3D::ERuntimeException("Failed to compile shader with id " + shaderId);
      }
    }
  ...
  void Shader::linkShaders() {
    ...
    if (isLinked == GL_FALSE) {
      ...
      std::printf("%s\n", &errorLog[0]);
      throw Ess3D::ERuntimeException("Failed to link program with id " + _programId);
    }
